...
export default function ReverseFunnelPage() {
  ...
  async function generateVisual() {
    ...
    try {
      ...
      if (data.success) {
        ...
        setError(data.error || 'Errore nella generazione visuale');
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Errore di rete');
    } finally {
  ...
  function downloadVisualHtml() {
    ...
    a.href = url;
    a.download = `funnel-rigenerato-${Date.now()}.html`;
    a.click();
  ...
  return (
    <div className="flex flex-col h-full">
      <Header
        ...
        subtitle="Analisi AI con reverse engineering — GPT-4.1 Vision + Multimodale"
      />
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="px-4 py-3 border-b border-slate-100 bg-gradient-to-r from-indigo-50 to-violet-50">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-sm font-bold text-slate-800">Materiali del Funnel</h2>
                  </div>
                  <p className="text-[10px] text-slate-400 mt-1">
                    Carica link, screenshot, PDF e note per l&apos;analisi AI
                  </p>
                </div>
                ...
                <div className="p-4 space-y-4 max-h-[calc(100vh-320px)] overflow-y-auto">
                  ...
                  <div>
                    <label className="flex items-center gap-1.5 text-xs font-semibold text-slate-600 mb-1.5">
                      ...
                      Nome Funnel (opzionale)
                    </label>
                    <input
                      type="text"
                      value={funnelName}
                      onChange={(e) => setFunnelName(e.target.value)}
                      placeholder="Es. Quiz Funnel Skincare..."
                  ...
                  <div>
                    <label className="flex items-center gap-1.5 text-xs font-semibold text-slate-600 mb-1.5">
                      ...
                      Link Pagine ({uploadLinks.length})
                    </label>
                    <div className="flex gap-1.5">
                      <input
                        ...
                        placeholder="https://esempio.com/funnel"
                  ...
                  <div>
                    <label className="flex items-center gap-1.5 text-xs font-semibold text-slate-600 mb-1.5">
                      ...
                      Documenti PDF ({uploadDocuments.length})
                    </label>
                    <DropZone
                      accept=".pdf,.doc,.docx"
                      onFiles={handleDocumentFiles}
                      icon={FileText}
                      label="Trascina PDF qui"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Aggiungi contesto, osservazioni, dettagli sul funnel..."
                      rows={3}
                    ...
                    <div className="flex items-center flex-wrap gap-2 pt-1">
                      ...
                      {uploadImages.length > 0 && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-emerald-50 text-emerald-600 border border-emerald-100">
                          <ImageIcon className="h-2.5 w-2.5" /> {uploadImages.length} immagini
                        </span>
                      )}
                      {uploadDocuments.length > 0 && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-orange-50 text-orange-600 border border-orange-100">
                          <FileText className="h-2.5 w-2.5" /> {uploadDocuments.length} documenti
                        </span>
                      )}
                    </div>
                  )}
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex items-center justify-center min-h-[500px]">
                <div className="text-center px-8">
                  ...
                  <p className="mt-2 text-sm text-slate-400 max-w-lg">
                    {inputMode === 'upload'
                      ? 'Carica link, screenshot, PDF o qualsiasi materiale del funnel. GPT-4.1 Vision analizzerà tutto in profondità con reverse engineering.'
                      : 'Seleziona un funnel dalla lista per analizzarlo con AI. GPT-4.1 analizzerà ogni step identificando il meccanismo unico, i trigger psicologici e molto altro.'}
                  </p>
                  <div className="mt-6 flex items-center justify-center gap-3">
                    ...
                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-violet-50 text-violet-600 border border-violet-100">
                      <ImageIcon className="h-3 w-3" /> Multimodale
                    </span>
                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-emerald-50 text-emerald-600 border border-emerald-100">
                      <Layout className="h-3 w-3" /> Visuale
                    </span>
                  </div>
                </div>
              </div>
            ...
            {analyzing && (
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex items-center justify-center py-24">
                <div className="text-center">
                  ...
                  <p className="mt-5 text-base font-bold text-slate-700">Reverse Engineering in corso...</p>
                  <p className="mt-1 text-sm text-slate-400">
                    GPT-4.1 sta analizzando {inputMode === 'upload' ? 'i materiali caricati' : 'ogni step del funnel'}
                  </p>
                  {inputMode === 'upload' && (
                    <div className="mt-3 flex items-center justify-center flex-wrap gap-2">
                      {uploadLinks.length > 0 && (
                        <span className="text-[10px] text-blue-500">
                          Recuperando {uploadLinks.length} link...
                        </span>
                      )}
                      {uploadImages.length > 0 && (
                        <span className="text-[10px] text-emerald-500">
                          Analizzando {uploadImages.length} immagini...
                        </span>
                      )}
                    </div>
                  )}
            ...
            {analysis && !analyzing && (
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-1 bg-white rounded-xl border border-slate-200 p-1 shadow-sm">
                  {(
                    [
                      { key: 'overview', label: 'Overview Globale' },
                      { key: 'steps', label: 'Analisi Step' },
                      { key: 'visual', label: 'Funnel Rigenerato', badge: !visualHtml },
                    ...
                    {analysis.regenerated_funnel && (
                      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                        <div className="px-5 py-3 border-b border-slate-100 bg-gradient-to-r from-indigo-50 to-purple-50">
                          <div className="flex items-center gap-2">
                            <Wand2 className="h-4 w-4 text-indigo-600" />
                            <h4 className="text-sm font-bold text-slate-800">Funnel Rigenerato — Concept</h4>
                          </div>
                        </div>
                        <div className="p-5 space-y-4">
                          ...
                          {analysis.regenerated_funnel.improvements_applied?.length > 0 && (
                            <div>
                              <p className="text-xs text-slate-400 uppercase tracking-wider mb-2">Miglioramenti Applicati</p>
                    ...
                    {generatingVisual && (
                      <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex items-center justify-center py-20">
                        <div className="text-center">
                          ...
                          <p className="mt-4 text-sm font-semibold text-slate-700">Generando mockup visuale...</p>
                          <p className="mt-1 text-xs text-slate-400">GPT-4.1 sta creando il layout del funnel rigenerato</p>
                          <div className="mt-4 flex items-center justify-center gap-1">
                            {[0, 1, 2, 3, 4].map((i) => (
                              <div
                                key={i}
                      ...
                      <>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-sm font-semibold text-slate-700">Mockup Funnel Rigenerato</p>
                          </div>
                          <div className="flex items-center gap-2">
                            <button
                              onClick={generateVisual}
                        ...
                        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                          <iframe
                            ...
                            title="Funnel Rigenerato Preview"
                          />
                        </div>
                      </>
                    )}
                    ...
                    {!visualHtml && !generatingVisual && (
                      <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex items-center justify-center py-20">
                        <div className="text-center px-8">
                          ...
                          <h4 className="mt-4 text-base font-bold text-slate-700">Mockup Visuale</h4>
                          <p className="mt-2 text-xs text-slate-400 max-w-md">
                            Genera un mockup HTML interattivo del funnel rigenerato basato sull&apos;analisi.
                          </p>
                          <button
                            onClick={generateVisual}

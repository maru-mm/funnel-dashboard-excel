...
REGOLA CRITICA DI OUTPUT: La tua risposta DEVE iniziare DIRETTAMENTE con il carattere { e terminare con }. NON aggiungere NULLA prima o dopo il JSON — nessun testo, nessuna spiegazione, nessun markdown, nessun blocco di codice. SOLO l'oggetto JSON puro.

L'oggetto JSON deve avere ESATTAMENTE queste chiavi:

{
  ...
  "category": "UNO tra: weight_loss | supplements | skincare | fitness | finance | saas | ecommerce | health | education | dating | real_estate | crypto | spirituality | astrology | other",
  ...
  "analysis_summary": "Paragrafo di analisi del funnel in italiano (2-4 frasi).",
  ...
  "notable_elements": ["elemento 1", "elemento 2"]
}
...
- I tags devono essere specifici e utili per filtrare
- L'analysis_summary deve essere un'analisi professionale in italiano
- Identifica TUTTE le tecniche di persuasione usate
- I notable_elements sono caratteristiche di design/UX notevoli`;

// =====================================================
// JSON PARSING — Multi-strategy, impossibile che fallisca
// =====================================================
...
// =====================================================
// FALLBACK PARSER — Estrae dati direttamente dal testo
// senza bisogno di Claude. Gestisce QUALSIASI formato.
// =====================================================
...
function regexFallbackParse(text: string, saveType?: 'quiz' | 'funnel'): Record<string, unknown> {
  ...
  if (steps.length > 20) notable.push(`Funnel lungo con ${steps.length} step`);
  if (hasQuizSteps) notable.push('Quiz interattivo con domande a scelta');
  if (/palm|scan|body|mano|palmo/i.test(text)) notable.push('Palm scan / body mapping interattivo');
  if (/progress/i.test(text)) notable.push('Progress bar durante il quiz');
  if (/email/i.test(text)) notable.push('Lead capture via email');
  if (/checkout|payment|pagamento|trial|prezzo|price|\$\d+/i.test(text)) notable.push('Checkout / pagamento nel funnel');
  if (/info.*screen|feedback.*intermedi|motivazion/i.test(text)) notable.push('Schermate informative / motivazionali intermedie');
  const inputTypes = new Set(steps.map(s => s.input_type).filter(t => t !== 'none'));
  if (inputTypes.size >= 3) notable.push(`Mix di ${inputTypes.size} tipi di input diversi`);
  ...
  const analysisSummary = `Funnel analizzato con ${steps.length} step totali${hasQuizSteps ? ', struttura quiz interattivo' : ''}. ` +
    `${techniques.length > 0 ? `Tecniche di persuasione individuate: ${techniques.join(', ')}. ` : ''}` +
    `${leadCapture !== 'none' ? `Lead capture tramite ${leadCapture}. ` : ''}` +
  ...
  try {
    ...
    if (!agentResult || agentResult.trim().length < 20) {
      return NextResponse.json(
        { success: false, error: 'agentResult è obbligatorio e deve contenere dati sufficienti.' },
        { status: 400 }
      );
    }
    ...
    if (apiKey) {
      const saveContext = saveType === 'quiz'
        ? 'L\'utente vuole salvare questo come QUIZ FUNNEL. Classifica il funnel_type come "quiz_funnel".'
        : saveType === 'funnel'
          ? 'L\'utente vuole salvare questo come FUNNEL DI VENDITA. Classifica il funnel_type in base al contenuto.'
          : 'Classifica il funnel_type in base al contenuto.';
    ...
    return NextResponse.json({
      ...
      usedFallback,
      message: `Funnel "${saved.funnel_name}" salvato con successo come ${saved.funnel_type} (${saved.category})${usedFallback ? ' [regex fallback]' : ''}`,
    });
  } catch (error) {
    ...
    try {
      ...
      if (typeof agentResult === 'string' && agentResult.length > 20) {
        ...
        const saved = await createAffiliateSavedFunnel({
          ...
          total_steps: steps.length,
          steps: steps as unknown as import('@/types/database').Json,
          analysis_summary: 'Salvataggio di emergenza — analisi Claude non disponibile.',
        ...
        return NextResponse.json({
          ...
          usedFallback: true,
          emergencySave: true,
          message: `Funnel "${saved.funnel_name}" salvato in modalità emergenza (${saved.total_steps} step estratti)`,
        });
      }
    } catch (emergencyErr) {
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Errore critico nel salvataggio',
      },
      { status: 500 }
    );
  }

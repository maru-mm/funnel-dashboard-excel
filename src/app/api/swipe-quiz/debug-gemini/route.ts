...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!entryUrl) {
      return NextResponse.json({ error: 'entryUrl obbligatorio' }, { status: 400 });
    }
    ...
    if (screenshots.length === 0) {
      return NextResponse.json({ error: 'Nessuno screenshot disponibile' }, { status: 400 });
    }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Errore debug Gemini' },
      { status: 500 },
    );
  }
}

...
/**
 * Polling per stato e risultato del browser agentico.
 */
export async function GET(
  ...
  try {
    ...
    if (!job) {
      return NextResponse.json(
        { success: false, error: 'Job non trovato', status: 'not_found' },
        { status: 404 }
      );
    }
    ...
    console.error('Browser agentico status error:', error);
    return NextResponse.json(

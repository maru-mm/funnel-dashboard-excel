...
/**
 * Health & diagnostics endpoint - testa connettività e configurazione
...
export async function GET() {
  ...
  // 4. Test Supabase connessione (usa tabella comune)
  if (supabaseUrl && supabaseKey) {
    try {
      ...
      checks.supabase_connect = {
        ok: !error,
        message: error ? `Errore: ${error.message}` : 'Connessione OK',
      };
    } catch (e) {
      checks.supabase_connect = {
        ok: false,
  ...
  // 5. Test esterno copy_analyzer (opzionale, può essere lento)
  try {
  ...
  return NextResponse.json({
    ...
    checks,
    hints: {
      funnel_analyzer_crawl: 'Richiede Playwright/Chromium. Su Fly.io può richiedere 60-90s.',

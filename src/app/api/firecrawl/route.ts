...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        { success: false, error: 'API Key Firecrawl è richiesta' },
        { status: 400 }
      );
    }
    ...
    if (!response.ok) {
      return NextResponse.json(
        { 
          success: false, 
          error: data.error || `Errore Firecrawl: ${response.status}`,
    ...
    console.error('Errore chiamata Firecrawl:', error);
    return NextResponse.json(
      { 
        success: false, 
        error: error instanceof Error ? error.message : 'Errore durante la chiamata a Firecrawl' 
      },
      { status: 500 }
    );
  }

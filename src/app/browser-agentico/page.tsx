    ...
    try {
      ...
      if (!startRes.ok || !startData.jobId) {
        setResult({
          ...
          totalSteps: 0,
          durationMs: 0,
          error: (startData as { error?: string }).error || 'Impossibile avviare',
        });
        setLoading(false);
        return;
      }
      ...
      const pollStatus = async (): Promise<void> => {
        ...
        if (statusData.status === 'failed') {
          setResult({
            ...
            error: statusData.error || 'Browser agentico fallito',
      ...
      setResult({
        ...
        totalSteps: 0,
        durationMs: 0,
        error: msg === 'Failed to fetch' ? 'Errore di rete' : msg,
      });
      setLiveSteps([]);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header
        ...
        subtitle="Gemini Computer Use + Playwright — il modello vede lo schermo e naviga il funnel autonomamente"
      />
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <div className="bg-violet-50 rounded-lg p-3 mb-4 border border-violet-200">
            <p className="text-sm text-violet-800">
              ...
              Il modello <strong>vede lo schermo</strong> tramite screenshot e genera azioni precise
              (click a coordinate, typing, scroll). Gestisce quiz, form, popup e naviga
              automaticamente fino al checkout.
            </p>
          </div>
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <span className="text-xs text-gray-400">Viewport: 1440 x 900 (raccomandato Google)</span>
          </div>
          ...
          <div className="flex gap-3">
            ...
            >
              ...
              {loading ? 'In esecuzione...' : 'Avvia Computer Use'}
            </button>
            {loading && (
              <button
                onClick={stopCrawl}
          ...
          <div className="bg-white rounded-lg shadow-sm border border-violet-200 p-4 mb-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-sm text-gray-500">
                — Computer Use analizza e agisce
              </span>
            </div>
...
function formatStopReason(reason: string): string {
  const map: Record<string, string> = {
    checkout_reached: 'Checkout raggiunto',
    task_complete: 'Task completato dal modello',
    no_actions_repeated: 'Nessuna azione per 3 turn',
    gemini_api_error: 'Errore API Gemini',
    blocked_payment_action: 'Azione pagamento bloccata',
    max_steps_reached: 'Max step raggiunto',
    user_stopped: 'Fermato dall\'utente',
    loop_ended: 'Loop terminato',
    exception: 'Errore imprevisto',
  };

...
export default function MobileFunnelPage() {
  ...
  const handleSaveAllFunnel = async () => {
    const name = funnelName.trim() || 'Funnel senza nome';
    setSaveAllError(null);
    setSaveAllSuccess(false);
    await runCrawl(async (data) => {
      if (!data.success || !data.steps?.length) {
        setSaveAllError('Nessuno step da salvare.');
        return;
      }
      setSaveLoading(true);
      try {
        ...
        setSaveAllError(err instanceof Error ? err.message : 'Errore salvataggio');
      } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-100 max-w-[480px] mx-auto shadow-xl">
      ...
      <header className="sticky top-0 z-10 bg-emerald-600 text-white px-4 py-3 safe-area-inset-top">
        ...
        <p className="text-emerald-100 text-xs mt-0.5">Incolla il link e salva gli step</p>
      </header>
      ...
      <div className="p-4 pb-8 space-y-4">
        ...
        {/* Nome funnel (usato per Salva tutto e per Salva selezionati) */}
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Nome funnel (per salvataggio)
          </label>
          <input
            type="text"
            value={funnelName}
        ...
        {saveAllSuccess && (
          <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-xl text-green-800 text-sm">
            ...
            Tutto il funnel è stato salvato.
          </div>
        )}
        {saveAllError && (
        ...
        {/* Lista step (dopo "Vedi tutti gli step") */}
        {view === 'steps' && result && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              ...
              <>
                <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between flex-wrap gap-2">
                  <span className="text-sm text-gray-600">
                    {result.totalSteps} step crawlati
                  </span>
            ...
            {result.success && result.steps.length === 0 && (
              <div className="p-6 text-center text-gray-500 text-sm">
                Nessuno step trovato per questo URL.
              </div>
            )}
          </div>
        )}
        ...
        <p className="text-xs text-gray-500 text-center pt-2">
          Pagina mobile · Link diretto: <strong>/m</strong>
        </p>
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 max-w-[480px] left-1/2 -translate-x-1/2">
          <div className="bg-white rounded-2xl p-6 mx-4 flex flex-col items-center shadow-xl">
            ...
            <p className="text-gray-900 font-medium">
              {loading ? 'Crawl in corso...' : 'Salvataggio...'}
            </p>
            <p className="text-gray-500 text-sm mt-1">
              {loading ? 'Analisi delle pagine del funnel' : 'Scrittura su Supabase'}
            </p>
          </div>
        </div>
      )}

...
function ColorSwatch({ color, label }: { color: string; label: string }) {
  ...
  return (
    <button onClick={handleCopy} className="flex flex-col items-center gap-1.5 group cursor-pointer" title={`Copia ${color}`}>
...
const PHASE_CONFIG = [
  { key: 'generate' as const, label: 'Replica Pagina', description: 'HTML pixel-perfect dallo screenshot', icon: Code },
  { key: 'review' as const, label: 'Review & Fix', description: 'Verifica fedeltà allo screenshot', icon: CheckCircle },
];
...
const SWIPE_PHASE_CONFIG = [
  { key: 'my-branding' as const, label: 'Il Mio Branding', description: 'Generazione branding personalizzato', icon: Wand2 },
  { key: 'swipe-regenerate' as const, label: 'Swipe & Rigenera', description: 'Nuova analisi swipata', icon: Repeat },
];
    ...
    <div className="space-y-6">
      ...
      {brandSummary && (
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl border border-amber-200 p-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            {brandSummary.conversion_strategy && <InfoRow label="Conversione" value={String(brandSummary.conversion_strategy)} />}
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <div className="grid grid-cols-2 gap-6">
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wider">Originale</p>
          ...
          <div>
            <p className="text-xs font-semibold text-amber-600 mb-2 uppercase tracking-wider">Swipata (Il Tuo Brand)</p>
            <div className="flex flex-wrap gap-3">
        ...
        {swiped.color_palette?.all_colors?.length > 0 && (
          <div className="mt-4 pt-4 border-t border-gray-100">
            <p className="text-xs text-gray-500 mb-2 font-medium">Tutti i colori del tuo brand</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <SectionCard icon={Target} title="Brand Identity (Swipato)" color="bg-amber-50 text-amber-700">
          <div className="space-y-0.5">
            <InfoRow label="Brand Name" value={swiped.brand_identity?.brand_name} />
            <InfoRow label="Personalità" value={swiped.brand_identity?.brand_personality} />
            <InfoRow label="Target Audience" value={swiped.brand_identity?.target_audience} />
            <InfoRow label="Settore" value={swiped.brand_identity?.industry} />
          </div>
        </SectionCard>

        <SectionCard icon={Type} title="Tipografia (Swipata)" color="bg-amber-50 text-amber-700">
          <div className="space-y-0.5">
            ...
            <InfoRow label="Pesi" value={swiped.typography?.font_weight_pattern} />
            <InfoRow label="Gerarchia" value={swiped.typography?.text_hierarchy} />
          </div>
        </SectionCard>

        <SectionCard icon={MousePointer} title="CTA (Swipate)" color="bg-amber-50 text-amber-700">
          <div className="space-y-3">
            <InfoRow label="CTA Principale" value={swiped.cta_analysis?.primary_cta_text} />
            <InfoRow label="Stile CTA" value={swiped.cta_analysis?.primary_cta_style} />
            <InfoRow label="Posizionamento" value={swiped.cta_analysis?.cta_placement} />
            {swiped.cta_analysis?.secondary_ctas?.length > 0 && (
              <div>
                <p className="text-xs text-gray-500 mb-1.5 font-medium">CTA Secondarie</p>
        ...
        <SectionCard icon={Sparkles} title="Quiz Funnel (Swipato)" color="bg-amber-50 text-amber-700">
          <div className="space-y-0.5">
            <InfoRow label="Tipo Quiz" value={swiped.quiz_funnel_elements?.quiz_type} />
            <InfoRow label="Stile Domande" value={swiped.quiz_funnel_elements?.question_style} />
            <InfoRow label="Formato Risposte" value={swiped.quiz_funnel_elements?.answer_format} />
            <InfoRow label="Progresso" value={swiped.quiz_funnel_elements?.progress_indicator} />
        ...
        <SectionCard icon={Layout} title="Layout (Swipato)" color="bg-amber-50 text-amber-700">
          <div className="space-y-3">
            <InfoRow label="Tipo Layout" value={swiped.layout_structure?.layout_type} />
            <InfoRow label="Hero Section" value={swiped.layout_structure?.hero_section} />
            <InfoRow label="Densità" value={swiped.layout_structure?.content_density} />
            {swiped.layout_structure?.sections?.length > 0 && (
              <div>
                <p className="text-xs text-gray-500 mb-1.5 font-medium">Sezioni</p>
        ...
        <SectionCard icon={TrendingUp} title="Valutazione (Swipata)" color="bg-amber-50 text-amber-700">
          <div className="space-y-4">
            <div className="space-y-3">
              <ScoreBar score={swiped.overall_assessment?.design_quality_score ?? 0} label="Qualità Design" />
              <ScoreBar score={swiped.overall_assessment?.modernity_score ?? 0} label="Modernità" />
              <ScoreBar score={swiped.overall_assessment?.conversion_optimization_score ?? 0} label="Conversioni" />
            </div>
            {swiped.overall_assessment?.key_strengths?.length > 0 && (
              <div>
                <p className="text-xs text-gray-500 mb-1.5 font-medium">Punti di Forza</p>
...
export default function QuizCreatorPage() {
  ...
  const runCodeGeneration = async (useSwipedAnalysis = false) => {
    ...
    try {
      ...
      if (!genRes.ok || !genData.success || !genData.code) {
        updatePhase('generate', 'error', { error: genData.error || 'Generazione fallita', duration_ms: genData.duration_ms });
      ...
      if (!revRes.ok || !revData.success || !revData.code) {
        ...
        updatePhase('review', 'error', { error: revData.error || 'Review fallita (usando codice pre-review)', duration_ms: revData.duration_ms });
  ...
  const runSwipeAnalysis = async () => {
    ...
    try {
      ...
      if (!bpRes.ok || !bpData.success || !bpData.myBranding) {
        updateSwipePhase('my-branding', 'error', { error: bpData.error || 'Generazione branding fallita', duration_ms: bpData.duration_ms });
      ...
      if (!swRes.ok || !swData.success || !swData.swipedAnalysis) {
        updateSwipePhase('swipe-regenerate', 'error', { error: swData.error || 'Swipe fallito', duration_ms: swData.duration_ms });
        setCurrentSwipePhase('error');
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header title="Quiz Creator" subtitle="Analisi Gemini + replica pixel-perfect HTML della pagina" />
        ...
        {loading && (
          <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-white rounded-2xl p-10 shadow-2xl flex flex-col items-center max-w-sm">
              ...
              <p className="text-gray-900 font-semibold text-lg">Analisi in corso...</p>
              <div className="mt-3 space-y-1.5 text-center">
                <p className="text-gray-500 text-sm">1. Screenshot della pagina</p>
                <p className="text-gray-500 text-sm">2. Analisi Gemini Vision AI</p>
                <p className="text-gray-500 text-sm">3. Estrazione branding e colori</p>
              </div>
              <p className="text-gray-400 text-xs mt-4">Questo processo può richiedere 15-30 secondi</p>
            </div>
          </div>
        )}
              ...
              <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div className="p-3 bg-gray-50">
                  <div className="max-h-[350px] overflow-y-auto rounded-lg border border-gray-200 shadow-inner">
                    ...
                    <img src={`data:image/png;base64,${result.screenshot}`} alt="Screenshot della pagina" className="w-full" />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl shadow-lg border border-gray-700 overflow-hidden flex flex-col">
                <div className="px-5 py-4 border-b border-gray-700">
                  ...
                  <p className="text-gray-400 text-xs mt-1">
                    Claude AI genera HTML pixel-perfect basandosi sullo screenshot e l&apos;analisi Gemini. Solo questa pagina, nessun quiz nuovo.
                  </p>
                </div>
                ...
                <div className="flex-1 p-5 flex flex-col justify-between">
                  <div className="space-y-3 text-sm">
                    <div className="flex items-center gap-2 text-gray-300">
                      ...
                      <span>Analisi Gemini = Blueprint visivo</span>
                    </div>
                    <div className="flex items-center gap-2 text-gray-300">
                      ...
                      <span>Claude replica in HTML/CSS/JS</span>
                    </div>
                    <div className="flex items-center gap-2 text-gray-300">
                      ...
                      <span>Review: confronto con screenshot</span>
                    </div>
                  </div>
            ...
            {genPhases.length > 0 && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                <h3 className="font-semibold text-gray-900 mb-4 flex items-center gap-2">
                  ...
                  {genSource === 'swiped' ? 'Replica Swipata' : 'Replica Pagina'} - Progresso
                  {genSource === 'swiped' && (
              ...
              <div className="bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 rounded-xl shadow-sm border border-amber-200 overflow-hidden">
                <div className="px-6 py-5 border-b border-amber-200">
                  ...
                  <p className="text-amber-700/70 text-sm mt-1">
                    Inserisci le info del tuo prodotto. Claude genererà il branding per il tuo brand ispirandosi all&apos;analisi originale, poi rigenerà una nuova analisi &quot;swipata&quot;.
                  </p>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Es. Fitness, Skincare, SaaS..."
                        className="w-full px-4 py-2.5 border border-amber-300 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none bg-white"
                        disabled={isSwiping}
                      />
                    </div>
                    <div className="md:col-span-2">
                      ...
                      <textarea
                        ...
                        placeholder="Descrivi brevemente il tuo prodotto o servizio..."
                        rows={2}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Es. Donne 25-45 che vogliono perdere peso"
                        className="w-full px-4 py-2.5 border border-amber-300 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none bg-white"
                        disabled={isSwiping}
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Es. Professionale, Amichevole, Energico..."
                        className="w-full px-4 py-2.5 border border-amber-300 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none bg-white"
                        disabled={isSwiping}
                      />
                    </div>
                    <div className="md:col-span-2">
                      ...
                      <textarea
                        ...
                        placeholder="Cosa rende unico il tuo prodotto? Es. 100% naturale, risultati in 30 giorni..."
                        rows={2}
                        className="w-full px-4 py-2.5 border border-amber-300 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none bg-white resize-none"
                        disabled={isSwiping}
                      />
                  ...
                  <div className="border-t border-amber-200">
                        ...
                        <div className="flex items-center gap-4">
                          <div className="flex-1">
                            <h4 className="font-bold text-amber-900 flex items-center gap-2">
                              ...
                              Genera HTML con il Tuo Branding
                            </h4>
                            <p className="text-amber-700/70 text-sm mt-0.5">
                              Stessa struttura dello screenshot originale, ma con i colori, testi e CTA del tuo brand swipato.
                            </p>
                          </div>
                          <button
                    ...
                    <div className="p-6">
                      ...
                      {swipeViewTab === 'branding-json' && myBranding && (
                        <div className="bg-gray-900 rounded-xl overflow-hidden">
                          <div className="px-5 py-2.5 bg-gray-800">
                            <span className="text-amber-400 text-sm font-mono">Il Mio Branding (Claude)</span>
                          </div>
                      ...
                      {swipeViewTab === 'swiped-json' && swipedAnalysisRaw && (
                        <div className="bg-gray-900 rounded-xl overflow-hidden">
                          <div className="px-5 py-2.5 bg-gray-800">
                            <span className="text-orange-400 text-sm font-mono">Analisi Swipata (Claude)</span>
                          </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                >
                  <h3 className="font-semibold text-gray-900 flex items-center gap-2">
                    ...
                    Analisi Branding Gemini Vision AI
                  </h3>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <SectionCard icon={Target} title="Brand Identity" color="bg-indigo-50 text-indigo-700">
                          <div className="space-y-0.5">
                            ...
                            <InfoRow label="Personalità" value={analysis.brand_identity?.brand_personality} />
                            <InfoRow label="Target Audience" value={analysis.brand_identity?.target_audience} />
                            <InfoRow label="Settore" value={analysis.brand_identity?.industry} />
                          </div>
                        </SectionCard>
                        ...
                        <SectionCard icon={Palette} title="Color Palette" color="bg-pink-50 text-pink-700">
                          <div className="space-y-4">
                            ...
                            {analysis.color_palette?.all_colors?.length > 0 && (
                              <div>
                                <p className="text-xs text-gray-500 mb-2 font-medium">Tutti i colori rilevati</p>
                        ...
                        <SectionCard icon={Type} title="Tipografia" color="bg-amber-50 text-amber-700">
                          <div className="space-y-0.5">
                            ...
                            <InfoRow label="Pesi" value={analysis.typography?.font_weight_pattern} />
                            <InfoRow label="Gerarchia" value={analysis.typography?.text_hierarchy} />
                          </div>
                        </SectionCard>

                        <SectionCard icon={Layout} title="Struttura Layout" color="bg-green-50 text-green-700">
                          <div className="space-y-3">
                            <div className="space-y-0.5">
                              <InfoRow label="Tipo Layout" value={analysis.layout_structure?.layout_type} />
                              <InfoRow label="Navigazione" value={analysis.layout_structure?.navigation_style} />
                              <InfoRow label="Hero Section" value={analysis.layout_structure?.hero_section} />
                              <InfoRow label="Densità" value={analysis.layout_structure?.content_density} />
                        ...
                        <SectionCard icon={Layers} title="Elementi Visivi" color="bg-cyan-50 text-cyan-700">
                          <div className="space-y-3">
                            <div className="space-y-0.5">
                              <InfoRow label="Immagini" value={analysis.visual_elements?.images_style} />
                              <InfoRow label="Icone" value={analysis.visual_elements?.icons_style} />
                              <InfoRow label="Bottoni" value={analysis.visual_elements?.buttons_style} />
                              <InfoRow label="Card" value={analysis.visual_elements?.cards_style} />
                              <InfoRow label="Animazioni" value={analysis.visual_elements?.animations_detected} />
                            </div>
                            {analysis.visual_elements?.decorative_elements?.length > 0 && (
                              <div>
                                <p className="text-xs text-gray-500 mb-1.5 font-medium">Elementi decorativi</p>
                        ...
                        <SectionCard icon={MousePointer} title="CTA Analysis" color="bg-orange-50 text-orange-700">
                          <div className="space-y-3">
                            <div className="space-y-0.5">
                              <InfoRow label="CTA Principale" value={analysis.cta_analysis?.primary_cta_text} />
                              <InfoRow label="Stile CTA" value={analysis.cta_analysis?.primary_cta_style} />
                              <InfoRow label="Posizionamento" value={analysis.cta_analysis?.cta_placement} />
                            </div>
                        ...
                        <SectionCard icon={Sparkles} title="Elementi Quiz / Funnel" color="bg-purple-50 text-purple-700">
                          <div className="space-y-0.5">
                            <div className="flex gap-2 py-1.5">
                              ...
                              <span className={`text-sm font-bold ${analysis.quiz_funnel_elements?.is_quiz_funnel ? 'text-green-600' : 'text-gray-400'}`}>
                                {analysis.quiz_funnel_elements?.is_quiz_funnel ? 'Sì' : 'No'}
                              </span>
                            </div>
                            <InfoRow label="Tipo Quiz" value={analysis.quiz_funnel_elements?.quiz_type} />
                            <InfoRow label="Stile Domande" value={analysis.quiz_funnel_elements?.question_style} />
                            <InfoRow label="Formato Risposte" value={analysis.quiz_funnel_elements?.answer_format} />
                            <InfoRow label="Progresso" value={analysis.quiz_funnel_elements?.progress_indicator} />
                            <InfoRow label="Step Rilevati" value={analysis.quiz_funnel_elements?.steps_detected != null ? String(analysis.quiz_funnel_elements.steps_detected) : null} />
                          </div>
                        </SectionCard>

                        <SectionCard icon={TrendingUp} title="Valutazione Complessiva" color="bg-rose-50 text-rose-700">
                          <div className="space-y-4">
                            <div className="space-y-3">
                              <ScoreBar score={analysis.overall_assessment?.design_quality_score ?? 0} label="Qualità Design" />
                              <ScoreBar score={analysis.overall_assessment?.modernity_score ?? 0} label="Modernità" />
                              <ScoreBar score={analysis.overall_assessment?.conversion_optimization_score ?? 0} label="Ottimizzazione Conversioni" />
                            </div>
                            ...
                            {analysis.overall_assessment?.design_style_tags?.length > 0 && (
                              <div>
                                <p className="text-xs text-gray-500 mb-1.5 font-medium">Stile Design</p>
                            ...
                            {analysis.overall_assessment?.improvement_suggestions?.length > 0 && (
                              <div>
                                <p className="text-xs text-gray-500 mb-1.5 font-medium">Suggerimenti di Miglioramento</p>
                                <ul className="space-y-1">
            ...
            {result.analysisRaw && !analysis && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-5 py-3 border-b border-gray-100 flex items-center gap-2.5">
                  ...
                  <h3 className="font-semibold text-gray-900">Risposta Gemini (raw - parsing JSON fallito)</h3>
                </div>

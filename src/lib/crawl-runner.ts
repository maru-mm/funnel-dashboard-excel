...
const NAV_TIMEOUT_MS = 120_000; // 2 min per page.goto (siti lenti tipo tryrosabella)
const SCREENSHOT_TIMEOUT_MS = 60_000; // 1 min per screenshot (evita blocchi su font/risorse lente)
    ...
    if (quizMode) {
      ...
      try {
        ...
        if (!response) {
          ...
          updateJob(jobId, {
            status: 'failed',
            error: 'Impossibile caricare la pagina quiz',
          });
          return;
        }
        ...
        const captureQuizStep = async (): Promise<FunnelCrawlStep> => {
          ...
          if (captureScreenshots) {
            try {
              ...
              // step senza screenshot se timeout (font/risorse lente)
            }
          }
        ...
        updateJob(jobId, {
          status: 'failed',
          error: err instanceof Error ? err.message : 'Errore durante il crawl quiz',
      ...
      try {
        ...
        if (captureScreenshots) {
          try {
            ...
            // step senza screenshot se timeout
          }
        }
